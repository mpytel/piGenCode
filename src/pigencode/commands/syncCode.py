...
def isDefaultStrCode(methodCode: List[str], className: str, initArgs: Dict) -> bool:
    ...
    # Default pattern includes: rtnStr = "ClassName = {\n" (with escaped newline)
    expectedStart1 = f'rtnStr = "{className} = ' + '{\\\\n"'  # Escaped newline
    expectedStart2 = f'rtnStr = "{className} = ' + '{\\n"'    # Literal newline
    
    if not (expectedStart1 in codeStr or expectedStart2 in codeStr):
        return False
    
    # Must contain standard elements for all init args
    argNames = list(initArgs.keys())
    for argName in argNames:
        # Check for the specific formatting pattern: f'    "argName":"{self.argName}",\n'
...
def shouldPreserveElegantPattern(seedContent: str, className: str, codeType: str, codeLines: List[str], options: dict = None) -> bool:
    ...
    if codeType == 'initAppendCode':
        ...
        if options and options.get('stats', False):
            if isDefault:
                ...
                printIt(f"SYNC: {codeType} for {className} - detected custom code", lable.DEBUG)
    ...
    if codeType == 'strCode':
        initArgs = extractInitArgsFromSeed(seedContent, className)
        isDefault = isDefaultStrCode(codeLines, className, initArgs)
        
        if options and options.get('stats', False):
            if isDefault:
                printIt(f"PRESERVE: {codeType} for {className} - detected default __str__ pattern", lable.DEBUG)
            else:
                printIt(f"SYNC: {codeType} for {className} - detected custom __str__ code", lable.DEBUG)
    ...
    if codeType == 'jsonCode':
        ...
        if options and options.get('stats', False):
            if isDefault:
                printIt(f"PRESERVE: {codeType} for {className} - detected default json() pattern", lable.DEBUG)
            else:
                printIt(f"SYNC: {codeType} for {className} - detected custom json() code", lable.DEBUG)
        
        return isDefault

    # For other code types, don't preserve (allow syncing)
    return False
