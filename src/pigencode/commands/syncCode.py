...
def updatePiSeedFileDirectory(piSeedFile: Path, className: str, piSeedType: str, dest_dir: str) -> bool:
    ...
    """
    print('in updatePiSeedFileDirectory')
    try:
...
def createNewPiDefGCSeedFileEnhanced(defName: str, pythonFile: Path, seed_file: Path | None = None, dest_dir: str | None = None) -> Optional[Path]:
    ...
    try:
        ...
        # print('createNewPiDefGCSeedFileEnhanced-fileDirectory',fileDirectory)
...
def createNewPiClassGCSeedFile(className: str, pythonFile: Path, seed_file: Path | None = None, dest_dir: Path | None = None) -> Optional[Path]:
...
def createNewPiDefGCSeedFile(defName: str, pythonFile: Path, seed_file: Path | None = None, dest_dir: Path | None = None) -> Optional[Path]:
...
def updateSeedImports(seedContent: str, className: str, regularImports: List[str]) -> Tuple[str, bool]:
    ...
    try:
            ...
            if re.match(importsPattern, line):
                if not foundImports:
                    ...
                    while i < len(lines) and re.match(importsPattern, lines[i]):
                        ...
                        match = re.match(
                            rf'^piValueA\s+{re.escape(className)}\.piBody:piClassGC:imports\s+(.+)$', lines[i])
                        if match:
                            existingImports.append(match.group(1))
                        i += 1
                    ...
                    if set(existingImports) != set(regularImports):
                        ...
                        for import_item in regularImports:
                            newLines.append(
                                f'piValueA {className}.piBody:piClassGC:imports {import_item}')
                        changed = True
                    else:
        ...
        if not foundImports and regularImports:
            ...
            for import_item in regularImports:
                newLines.insert(
                    insertIndex, f'piValueA {className}.piBody:piClassGC:imports {import_item}')
                insertIndex += 1
