...
def isDefaultGeneratedStrCode(methodCode: List[str], className: str) -> bool:
    """
                ...
                for item in classNode.body:
                    if isinstance(item, ast.FunctionDef):
                        ...
                        if methodName == '__init__':
                            ...
                            # IDEMPOTENCY FIX: Skip default generated __str__ methods unless forced
                            if not options.get('force', False):
                                # Extract the method code to check if it's default
                                methodCode = extractMethodCode(pythonContent, item)
                                if methodCode and isDefaultGeneratedStrCode(methodCode, className):
                                    if options.get('stats', False):
                                        printIt(f"SKIP: strCode for {className} - default generated __str__ method (idempotency)", lable.DEBUG)
                                    # Skip this sync entirely
                                    pass
                                else:
                                    # Not default - proceed with normal sync
